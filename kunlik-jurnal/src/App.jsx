...
import { supabase, getCurrentUser, signOut, fetchEntriesByDate, saveEntry, deleteEntry } from './lib/supabase'
...
export default function App() {
  ...
  function handleCellClick(hour, colKey) {
    if (!user) { setPendingCell({ hour, colKey }); setShowAuth(true) }
    else setActiveCell({ hour, colKey })
  }

  async function handleAuthSuccess(loggedUser, typedName) {
    setUser(loggedUser)
    const name = loggedUser.user_metadata?.full_name || typedName || loggedUser.email?.split('@')[0] || ''
    setUserName(name)
    setShowAuth(false)
    const data = await fetchEntriesByDate(loggedUser.id, selectedDate)
    setEntries(data)
    if (pendingCell) { setActiveCell(pendingCell); setPendingCell(null) }
  }
  ...
  async function handleSave(text) {
    ...
    if (text) { newEntries[key] = text; await saveEntry(user.id, selectedDate, hour, colKey, text) }
    else { delete newEntries[key]; await deleteEntry(user.id, selectedDate, hour, colKey) }
    setEntries(newEntries)
    setActiveCell(null)
  }

  async function handleLogout() {
    await signOut(); setUser(null); setUserName(''); setEntries({})
  }
  ...
  return (
    <div className="app">
      <header className="header">
        <div className="header-top">
          ...
          <div className="header-right">
            {user ? (
              ...
              <button className="btn-sm" onClick={() => setShowAuth(true)}>Кирүү</button>
            )}
          </div>
        </div>
        ...
        {user && (
          <div className="progress-wrap">
            ...
            <span className="progress-label">{filledCells} / {totalCells} уяча • {pct}%</span>
          </div>
        )}
      </header>
      ...
      <main className="main">
              ...
              {HOURS.map(h => (
                <tr key={h}>
                  ...
                  {COL_KEYS.map(ck => {
                    ...
                    return (
                      <td key={ck} className={`td-cell ${val ? 'filled' : ''}`}
        ...
        {user && <DayAnalysis entries={entries} hours={HOURS} cols={COLS} colKeys={COL_KEYS} date={selectedDate} />}
      </main>

      {showAuth && (
        <AuthModal onSuccess={handleAuthSuccess} onClose={() => { setShowAuth(false); setPendingCell(null) }} />
      )}
      {activeCell && (
        <EntryModal
